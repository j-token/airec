...
impl ContainerSettingsBuilder {
    ...
    pub const fn new() -> Self {
        Self { sub_type: ContainerSettingsSubType::MPEG4, hardware_acceleration: true }
    }
    ...
    /// Enables or disables Media Foundation hardware acceleration.
    pub const fn hardware_acceleration(mut self, enabled: bool) -> Self {
        self.hardware_acceleration = enabled;
        self
    }
    fn build(self) -> Result<(ContainerEncodingProperties, bool), VideoEncoderError> {
        let properties = ContainerEncodingProperties::new()?;
        properties.SetSubtype(&self.sub_type.to_hstring())?;
        Ok((properties, self.hardware_acceleration))
    }
}
...
pub enum ContainerSettingsSubType {
    ...
    MPEG4,
    /// Fragmented MPEG-4 container. Uses the Media Foundation FMPEG4 sink so completed
    /// fragments remain readable if the process is terminated before finalization.
    FMPEG4,
...
impl ContainerSettingsSubType {
    ...
    pub fn to_hstring(&self) -> HSTRING {
        match self {
            ...
            Self::FMPEG4 => HSTRING::from("9ba876f1-419f-4b77-a1e0-35959d9d4004"),
...
impl VideoEncoder {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        media_transcoder.SetHardwareAccelerationEnabled(hardware_acceleration)?;
